...
std::string vertexShaderSrc = R"(
	...
	void main()
	{
		gl_Position = vec4(aPos.x + ourPos, (aPos.y + ourPos) * ourPos - 0.5f, aPos.z, 1.0f);
		ourColor = aColor;
	}
)";
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float position = sin(time);
		int vertexPosition = glGetUniformLocation(shaderProgram, "ourPos");
		glUniform1f(vertexPosition, position);
