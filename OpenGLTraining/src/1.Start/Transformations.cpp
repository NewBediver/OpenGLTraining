...
// Update the third parameter of mix function
void updateVisibility(float updateValue);
...
int main()
{
	...
	// Create shader
	Shader ourShader("Shaders/5.1.Transformation.vs", "Shaders/5.1.Transformation.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform1f(glGetUniformLocation(ourShader.GetID(), "visibility"), visibility);

		// Lower right container -------------------------

		// Scale, rotate and then translate
		glm::mat4 trans = glm::mat4(1.0f);
		trans = glm::translate(trans, glm::vec3(0.65f, -0.65f, 0.0f));
...
void updateVisibility(float updateValue) {
	if (visibility + updateValue > 1.0f || visibility + updateValue < 0.0f) return;
